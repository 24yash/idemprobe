...
class ProbeRunnerTest {
    ...
    @Test
    void sendsSequentialDuplicatesWithOneHeaderKeyThenVerifies() {
        List<String> keys = new ArrayList<>();
        List<String> untouchedHeaders = new ArrayList<>();
        ...
        server.createContext("/reservations/count", exchange -> {
            verificationRanAfterDuplicates.set(keys.size() == 3);
        ...
        assertThat(keys).containsExactly(
                "prefix-shared-key-shared-key",
                "prefix-shared-key-shared-key",
                "prefix-shared-key-shared-key");
        assertThat(untouchedHeaders).containsOnly("${not.a.template}");
        assertThat(bodies).containsOnly("{\"probe\":\"${probe.key}\"}");
        assertThat(verificationRanAfterDuplicates).isTrue();
        assertThat(execution.invocations()).extracting(InvocationResult::index)
                .containsExactly(0, 1, 2);
        assertThat(execution.invocations()).extracting(InvocationResult::phase)
